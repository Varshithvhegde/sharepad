---
title: I Built a Multi-Page Markdown Notebook You Can Share Without Creating an Account
published: false
description: SharePad is a no-signup notebook app — paste markdown, get a link, add pages, lock it with a password, paste screenshots, export PDF. One view link to share, one secret edit link to keep.
tags: webdev, nextjs, markdown, opensource, supabase
# COVER — required. Upload to dev.to (Settings → Upload image) and paste the URL here.
# Recommended: split editor screenshot, 1000×420 or wider. See IMAGE 1 below.
cover_image: REPLACE_WITH_DEVTO_UPLOAD_URL
---

<!--
═══════════════════════════════════════════════════════════════════════════════
  IMAGE GUIDE — read this before publishing
═══════════════════════════════════════════════════════════════════════════════

  Upload each screenshot to dev.to (click the image icon in the editor, or
  drag onto the upload area). Replace every REPLACE_WITH_DEVTO_UPLOAD_URL below
  with the URL dev.to gives you (looks like dev-to-uploads.s3.us-east-2.amazonaws.com/...).

  PRIORITY KEY
  ★★★  Must have — publish without these and the post feels empty
  ★★   Strongly recommended — big payoff for little effort
  ★    Nice to have — add if you have time

  ┌──────┬──────────┬─────────────────────────────────────────────────────────┐
  │  #   │ Priority │  What to capture                                        │
  ├──────┼──────────┼─────────────────────────────────────────────────────────┤
  │  1   │ ★★★      │  COVER + hero — split editor (write + preview)          │
  │  2   │ ★★★      │  Quick paste flow — /quick with link copied toast       │
  │  3   │ ★★       │  Multi-page sidebar — 3+ pages visible in index         │
  │  4   │ ★★       │  Share panel — view link, edit link, QR code            │
  │  5   │ ★★       │  Home page — hero + “Free forever” pinned tag           │
  │  6   │ ★★       │  Print/PDF view — serif document, not handwritten UI    │
  │  7   │ ★        │  Image paste — screenshot mid-upload or rendered inline │
  │  8   │ ★        │  Settings panel — expiry, password, open editing        │
  │  9   │ ★        │  New notebook form — custom slug + paper texture pick   │
  │  10  │ ★        │  404 page — torn-paper “notebook missing” error         │
  │  11  │ ★        │  Terminal — npm run dev or successful deploy/build      │
  │  12  │ ★        │  GIF — paste screenshot → link appears in editor (loop) │
  └──────┴──────────┴─────────────────────────────────────────────────────────┘

  TIPS
  • Use a real notebook with content (meeting notes, todo list) — not lorem ipsum.
  • Crop browser chrome if it clutters; keep the SharePad UI obvious.
  • Dark mode off — the paper aesthetic is the point.
  • For IMAGE 12, record 5–8 sec with QuickTime → convert to GIF (ezgif.com).
  • Local file public/screenshot.png works for IMAGE 1 if you upload it to dev.to.
